...
import {ReturnRequest} from "../../ApiRequest/Return/ReturnApiRequest.js";
...
import {DeleteReturnRequest} from "../../ApiRequest/Return/ReturnDeleteApiRequest.js";
...
const ReturnList = () => {
    ...
    useEffect(()=> {
        ( async ()=>{
            setLoading(true);
            await ReturnRequest(1, perPage, searchKeyword);
            setLoading(false);
        })()
    }, []);

    const AllPReturn = useSelector((state) => state.return.list);
    const Total = useSelector((state) => state.return.listTotal);

    const handlePageClick = async (event) => {
        const pageNo = event.selected;
        await ReturnRequest(pageNo + 1, perPage, searchKeyword);
    };

    const perPageOnChange = async (event) => {
        setPerPage(parseInt(event.target.value));
        await ReturnRequest(1, event.target.value, searchKeyword);
    };
    ...
    const SearchKeywordOnChange = async (event) => {
        ...
        if (event.target.value.length === 0) {
            setSearchKeyword("0");
            await ReturnRequest(1, perPage, "0");
        }
    };
    ...
    const TextSearch = (e) => {
        ...
        rows.forEach((row) => {
            const returnNameCell = row.querySelector("td:nth-child(3) p"); // Target the name column (3rd column)
            if (returnNameCell) {
                const returnName = returnNameCell.textContent.toLowerCase(); // Convert to lowercase
                row.style.display = returnName.includes(searchValue) ? "" : "none";
            }
        });
    };
                                        ...
                                        <div className="container-fluid">
                                            <div className="row mb-4">
                                                <div
                                                    ...
                                                    <h4 className="fw-bold text-success">Returns List</h4>
                                                </div>
                                            </div>
                                            <div className="row">
                                                <div className="col-4">
                                                    <input onKeyUp={TextSearch} placeholder="Filter returns.."

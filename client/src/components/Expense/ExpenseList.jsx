...
const ExpenseList = () => {
    ...
    useEffect(()=> {
        ( async ()=>{
            setLoading(true);
            await ExpenseRequest(1, perPage, searchKeyword);
            setLoading(false);
        })()
    }, []);

    const AllExpense = useSelector((state) => state.expense.list);
    const Total = useSelector((state) => state.expense.listTotal);

    const handlePageClick = async (event) => {
        const pageNo = event.selected;
        await ExpenseRequest(pageNo + 1, perPage, searchKeyword);
    };

    const perPageOnChange = async (event) => {
        setPerPage(parseInt(event.target.value));
        await ExpenseRequest(1, event.target.value, searchKeyword);
    };
    ...
    const SearchKeywordOnChange = async (event) => {
        ...
        if (event.target.value.length === 0) {
            setSearchKeyword("0");
            await ExpenseRequest(1, perPage, "0");
        }
    };
    ...
    const TextSearch = (e) => {
        ...
        rows.forEach((row) => {
            const expenseNameCell = row.querySelector("td:nth-child(3) p");
            if (expenseNameCell) {
                const expenseName = expenseNameCell.textContent.toLowerCase();
                row.style.display = expenseName.includes(searchValue) ? "" : "none";
            }
        });
    };
    ...
    const getEditRoute = (role) => {
        switch (role) {
            case "superadmin":
                return "superadminexpensecreateupdate";
            case "admin":
                return "adminexpensecreateupdate";
            case "employee":
                return "employeeexpensecreateupdate";
            default:
                return null;
        }
    };
    ...
    const deleteItem = async (id) => {
        ...
        if(result.isConfirmed){
            let res = await DeleteExpenseRequest(id);
                                        ...
                                        <div className="container-fluid">
                                            <div className="row mb-4">
                                                <div
                                                    ...
                                                    <h4 className="fw-bold text-success">Expenses List</h4>
                                                </div>
                                            </div>
                                            <div className="row">
                                                <div className="col-4">
                                                    <input onKeyUp={TextSearch} placeholder="Filter expenses.."

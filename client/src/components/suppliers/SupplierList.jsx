...
import {DeleteSupplierRequest} from "../../ApiRequest/Suppliers/SupplierDeleteApiRequest.js";
...
const SupplierList = () => {
    ...
    useEffect(()=> {
        ( async ()=>{
            setLoading(true);
            await SupplierRequest(1, perPage, searchKeyword);
            setLoading(false);
        })()
    }, []);

    const AllSupplier = useSelector((state) => state.supplier.list);
    const Total = useSelector((state) => state.supplier.listTotal);

    const handlePageClick = async (event) => {
        const pageNo = event.selected;
        await SupplierRequest(pageNo + 1, perPage, searchKeyword);
    };

    const perPageOnChange = async (event) => {
        setPerPage(parseInt(event.target.value));
        await SupplierRequest(1, event.target.value, searchKeyword);
    };
    ...
    const SearchKeywordOnChange = async (event) => {
        ...
        if (event.target.value.length === 0) {
            setSearchKeyword("0");
            await SupplierRequest(1, perPage, "0");
        }
    };
    ...
    const TextSearch = (e) => {
        ...
        rows.forEach((row) => {
            const supplierNameCell = row.querySelector("td:nth-child(3) p"); // Target the name column (3rd column)
            if (supplierNameCell) {
                const supplierName = supplierNameCell.textContent.toLowerCase(); // Convert to lowercase
    ...
    const getEditRoute = (role) => {
        switch (role) {
            case "superadmin":
                return "superadminsuppliercreateupdate";
            case "admin":
                return "adminsuppliercreateupdate";
            case "employee":
                return "employeesuppliercreateupdate";
            default:
                return null;
        }
    };
    ...
    const deleteItem = async (id) => {
        ...
        if(result.isConfirmed){
            let res = await DeleteSupplierRequest(id);
                                        ...
                                        <div className="container-fluid">
                                            <div className="row mb-4">
                                                <div
                                                    ...
                                                    <h4 className="fw-bold text-success">Suppliers List</h4>
                                                </div>
                                            </div>
                                            <div className="row">
                                                <div className="col-4">
                                                    <input onKeyUp={TextSearch} placeholder="Filter suppliers.."
